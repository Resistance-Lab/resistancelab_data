...
def save_disability():
    ...
    mapping = pd.read_csv(os.path.join(directory, "..", "..", "..", "mappings", "use-of-force.csv")).set_index(
        "Tactic - Input")
    cleansed = cleansed.join(mapping, on=["Tactic"]).drop("Type of force - Input", axis=1)
    cleansed.to_csv(os.path.join(directory, "april2018-march2019.csv"), index=False)

    force_by_disability_rebuilt = cleansed.drop(columns=["Number of times tactic reported", "Type of force"])
    force_by_disability_melted = force_by_disability_rebuilt.melt(
        ["Tactic", "Police Force", "Force type - Output", "Tactic - Output"], var_name="Disability",
        value_name="count")
    force_by_disability_melted.to_csv(os.path.join(directory, "april2018-march2019-melted.csv"), index=False)
...
def save_ethnicity():
    ...
    force_by_ethnicity_rebuilt = cleansed.drop(columns=["Number of times tactic reported", "Type of force"])
    force_by_ethnicity_melted = force_by_ethnicity_rebuilt.melt(["Tactic", "Police Force", "Force type - Output", "Tactic - Output"], var_name="Ethnicity",
                                                                value_name="count")
    force_by_ethnicity_melted.to_csv(os.path.join(directory, "april2018-march2019-melted.csv"), index=False)
...
def save_age():
    ...
    force_by_age_rebuilt = cleansed.drop(columns=["Number of times tactic reported", "Type of force"])
    force_by_age_melted = force_by_age_rebuilt.melt(["Tactic", "Police Force", "Force type - Output", "Tactic - Output"], var_name="Age",
                                                    value_name="count")
    force_by_age_melted.to_csv(os.path.join(directory, "april2018-march2019-melted.csv"), index=False)
